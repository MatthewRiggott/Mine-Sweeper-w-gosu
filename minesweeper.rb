...
class Minesweeper < Gosu::Window
  ...
  def draw
    ...
    (1...field.row_count+1).each do |row|
      (1...field.column_count+1).each do |col|
        x = start_x + ((col-1) * cell_size)
        y = start_y + ((row-1) * cell_size)
  ...
  def cell_size
    max_cell_width = (screen_width) / field.column_count
    max_cell_height = (screen_height) / field.row_count
